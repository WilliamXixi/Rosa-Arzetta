	...
	key : {
       ...
       message: {
                    orderMessage: {
                      ...
                      const sendImage = (from, image, caption, mek) => {
	hexa.sendMessage(from, image, MessageType.image, {quoted: mek, caption: caption})
}
...
case 'add':
					...
					break
case 'tovideo': 
				if (!isQuotedSticker) return reply(from, 'Reply stiker nya', mek)
				if (mek.message.extendedTextMessage.contextInfo.quotedMessage.stickerMessage.isAnimated === true){
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) {
							reply(from, `gagal`, mek)
							fs.unlinkSync(ran)
						} else {
							buffer = fs.readFileSync(ran)
							sendImage(from, buffer, 'nih', mek)
							fs.unlinkSync(ran)
						}
					})
				}
...
case 'tiktoknowm':
  if (args.length < 1) return reply('*Masukan Url nya?*')
					query = args.join(" ")
					reply(mess.wait)
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/tiktok?apikey=0c725e946703940de2101893&url=${query}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					tt = `「 *TIKTOK NO WM* 」\n\n*Judul:* ${anu.result.title}\n*Keywords:* ${anu.result.keywords}\n*Desc:* ${anu.result.description}`
 buff = await getBuffer(anu.result.link)
 hexa.sendMessage(from, buff, video, {mimetype: 'video/mp4', quoted: mek,caption : tt})
					break
case 'anu':
				res = await hexa.prepareMessageFromContent(from, {
				             "groupInviteMessage": {
...
- *Link Dev* : ${i.link_dev}\n❉─────────────────────❉`
    ...
    case 'kontak':
            ...
            case 'takestick':
			case 'take':
				if (!isQuotedSticker) return reply(from, `Reply sticker dengan caption *${prefix}takestick nama|author*`, mek)
				const pembawm = body.slice(11)
				if (!pembawm.includes('|')) return reply(from, `Reply sticker dengan caption *${prefix}takestick nama|author*`, mek)
				const encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
				const media = await hexa.downloadAndSaveMediaMessage(encmedia, `./stik/${sender}`)
				const packname = pembawm.split('|')[0]
				const author = pembawm.split('|')[1]
				exif.create(packname, author, `takestick_${sender}`)
				exec(`webpmux -set exif ./stik/takestick_${sender}.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
...
case 'kick':
                        ...
                        break
case 'add':
			if (!mek.key.fromMe) return reply(from, 'ANDA BUKAN OWNER', mek)
				if (!arg) return reply(from, `Penggunaan ${prefix}kick 628xxxx`, mek)
				add(from, [args[0] + '@s.whatsapp.net'])
				fakethumb(from, `Sukses`, mek)
				break
    case 'toimg':
